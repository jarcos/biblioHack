...
| **M4 — Goodreads import** | CSV importer, matching to catalog, bookshelf UI. **(Slice 1 shipped 2026-06-08:** `reading_history` context + `shelf_entries` table; `bibliohack shelf import <csv>` parses a Goodreads export and matches each book to the catalogue by ISBN-13 first, then a conservative pg_trgm title+author fallback, keeping unmatched rows for free re-matching as the catalogue grows. Single-user (no auth). ISBN-13 normalization is now shared with ingest via `catalog/domain/isbn.py`. Slices 2 (shelf read API) + 3 (bookshelf UI) pending.) | 1–2 weekends |
