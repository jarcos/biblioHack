...
---
  ...
  <div class="panel">
    <p>The product targets <strong>all eight RBPA provinces</strong>, not just Huelva; every user picks their own library of reference. The Libraries milestone already made branch-following region-wide (571/573 branches geocoded + followable, scope «mis bibliotecas → mi provincia → todo»), so the gating piece is the <strong>catalogue crawl</strong>: bibliographic discovery is still Huelva-seeded via <code>SUBC</code>, so records held only in other provinces aren't mirrored yet (holdings/copies <em>are</em> stored network-wide). Two pieces close the gap:</p>
    <p><strong>M7 — all-province crawl (NEXT PRIORITY).</strong> Generalise the Huelva-only discovery scoping to a config-driven province set; crawl Sevilla, Málaga, Cádiz… within the 1 req/s politeness budget (so expect weeks of polite crawl — phase province-by-province). Crawler-plane change → manual NAS rebuild. This is the prerequisite for publicising region-wide.</p>
