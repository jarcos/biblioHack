...
| Catalog | URL | Notes |
| --- | --- | --- |
| RBPA collective OPAC | <https://www.juntadeandalucia.es/cultura/absys/abnopac/abnetcl.cgi?ACC=101> | Federated and **network-wide**: searches return records from all eight provinces. `SUBC` is a *display* filter (which branch's holdings to foreground), not a search filter, so discovery was never province-scoped. M7's region-wide goal is about *crawl coverage* (the pre-2024 backlist), not SUBC handling. |
...
| **M7 — Network-wide backlist crawl (region-wide)** | **NEXT PRIORITY (re-prioritised 2026-06-25; framing corrected 2026-06-26).** The prerequisite for publicising the app region-wide. **Correction:** discovery is *not* Huelva-scoped and there is no `SUBC` filter to "generalise" — the OPAC expert queries (`@fepu` publication-year slices, TITN enumeration) already return results network-wide, `SUBC` only scopes *display* not search (§2.3 / `urls.py`), and ingest already stores copies for all branches. The real gap is **coverage, not scoping**: today's mirror ≈ the one-time bootstrap TITN sweep range + the ongoing novedades discovery (`@fepu>=2024`). The **pre-2024 backlist, in every province**, is only as complete as the bootstrap sweep got, so older titles held anywhere in the network aren't mirrored yet. Closing it is **crawl-time-bound** by the §6.3 politeness budget (1 req/s) — weeks of polite crawl — not a config feature; phase by TITN range / publication-year slice (not by province) and watch the crawl dashboards. Crawler-plane change → manual NAS rebuild (crawler ≠ CD). | several weekends + crawl time |
